...
再インデックスと代表ベクトル検索には BWS の `GRIMOIRE_KEEPER_OPENAI_API_KEY` が必要です。
`migrate.sh` は停止・バックアップ前にキーが空でないことを値を出力せず確認します。
再インデックスは `bws run` 配下の本番 Compose の API サービスで実行し、
`OPENAI_API_KEY` への変換を継承します。一時ツールコンテナも同じ BWS キーを使用します。
実キーを `.env`、テスト、ログに記録しないでください。
