...
class TestPageRepository:
    ...
    def test_create_page_sync(self, page_repo: Any) -> None:
        """ページ作成テスト（同期版）."""
        url = "https://example.com"
        title = "Test Title"
        memo = "Test memo"

        page_id = page_repo.create_page_sync(url, title, memo)
        assert page_id is not None
        assert isinstance(page_id, int)

    def test_get_page_sync(self, page_repo: Any) -> None:
        """ページ取得テスト（同期版）."""
        url = "https://example.com"
        title = "Test Title"
        memo = "Test memo"
        ...
        # ページ取得
        page = page_repo.get_page_sync(page_id)
    ...
    def test_get_nonexistent_page_sync(self, page_repo: Any) -> None:
        """存在しないページの取得テスト（同期版）."""
        page = page_repo.get_page_sync(999)
        assert page is None

    def test_get_page_by_url_sync(self, page_repo: Any) -> None:
        """URLでページ取得テスト（同期版）."""
        url = "https://example.com"
        title = "Test Title"
        memo = "Test memo"
        ...
        # URLでページ取得
        page = page_repo.get_page_by_url_sync(url)
        assert page is not None
        assert page.id == page_id
        assert page.url == url
        assert page.title == title
    ...
    def test_get_page_by_nonexistent_url_sync(self, page_repo: Any) -> None:
        ...
        page = page_repo.get_page_by_url_sync("https://nonexistent.com")
        assert page is None
    ...
    async def test_update_summary_keywords(self, page_repo: Any) -> None:
        """要約・キーワード更新テスト."""
        # ページ作成
        page_id = page_repo.create_page_sync("https://example.com", "Test Title")
    ...
    async def test_update_page_title(self, page_repo: Any) -> None:
        """ページタイトル更新テスト."""
        # ページ作成
        page_id = page_repo.create_page_sync("https://example.com", "Old Title")
    ...
    async def test_get_all_pages(self, page_repo: Any) -> None:
        ...
        page_ids = []
        for i in range(3):
            page_id = page_repo.create_page_sync(
    ...
    async def test_get_all_pages_with_limit(self, page_repo: Any) -> None:
        ...
        # 5ページ作成
        for i in range(5):
            page_repo.create_page_sync(f"https://example{i}.com", f"Test Title {i}")
