...
class TestSearchServiceKeywords:
    ...
    def test_build_weaviate_filter_keywords_string(self, search_service: SearchService) -> None:
        """文字列キーワードフィルター構築テスト."""
        ...
        with patch('weaviate.classes.query.Filter') as mock_filter:
            ...
            mock_filter.by_property.return_value.contains_any.assert_called_with(["single_keyword"])
            assert result == "keywords_filter"

    def test_build_weaviate_filter_keywords_empty_list(self, search_service: SearchService) -> None:
        """空配列キーワードフィルター構築テスト."""
        filters = {"keywords": []}
    ...
    def test_build_weaviate_filter_keywords_empty_string(self, search_service: SearchService) -> None:
        """空文字列キーワードフィルター構築テスト."""
        filters = {"keywords": ""}
