...
検索対象の属性・状態はSQLiteを正とし、共通の検索条件で対象IDの事前取得と
候補の再確認を行います。Weaviateの古いキーワードやページ属性では絞り込みません。
削除中・再処理中のページは再確認時に除外しますが、分散トランザクションでは
ないため、その後の状態変更や検索開始後に条件一致となったページは反映されない
場合があります。

対象ID数をNとするとIDのメモリ使用量はO(N)、検索グループ数はceil(N/100)です。
各グループのWeaviate呼び出しは通常1回、候補が途中で検索不可になった場合などは
最大10回です。並列呼び出しは増やさず、グループの処理後に上位limit件だけを保持
します。広い検索ほど呼び出し数と待ち時間が増えます。全体の候補1000件で止める
方式は使用しません。SQLiteのURL部分一致・JSONキーワード条件もデータ量に応じた
走査コストがあります。上限による不完全な応答はAPIのtruncatedとWebの案内で示します。

IDフィルターの実接続テストは、埋め込み済みの小さなベクトルを持つ一時コレクションを
作り、3種類のnamed vectorに対するフィルターと候補収集を検証します。
OpenAIへの埋め込み要求は実行しません。既存のスキーマテストと併せて実行する例:

```bash
WEAVIATE_INTEGRATION=1 WEAVIATE_INTEGRATION_EMBEDDED=1 \
WEAVIATE_EMBEDDING_MODEL=text-embedding-3-small \
uv run pytest apps/api/tests/integration/services/test_weaviate_1_38_schema.py -v
```

Weaviate 1.38.8は既定の `text-embedding-ada-002` というモデル名をスキーマ作成時に
拒否するため、このテスト例では対応モデルを環境変数で指定しています。
本番のモデル変更・再インデックスはこの検証に含みません。
