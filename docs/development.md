...
本番反映前にAPIとworkerを停止し、SQLiteファイルのバックアップを取得します。現在の
実装はアップグレードのみを提供し、down migrationは提供しません。新しいスキーマへ
移行した後に旧アプリへ戻す場合は、コードだけでなくSQLiteファイルも同じ時点の
バックアップへ戻してください。旧アプリが新しいスキーマを読み書きできるとは
仮定しません。
